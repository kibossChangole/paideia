    ...
    try {
      ...
      if (selectedStudent === "all") {
        ...
        for (const student of students) {
          const studentDiaryRef = ref(
            db,
            `diary/${schoolCode}/${currentGrade}/${student.id}`
          );
          ...
          if (entriesSnapshot.exists()) {
            ...
            const studentEntries = Object.entries(entriesData)
              ...
              .filter((entry) => {
                ...
                return (
                  entryDateString >= startOfDay && entryDateString < endOfDay
                );
              });
        ...
        const selectedStudentRef = ref(
          db,
          `diary/${schoolCode}/${currentGrade}/${selectedStudent}`
        );
      ...
      if (selectedStudent === "all") {
        ...
        for (const student of students) {
          try {
            ...
            console.error(
              `Error adding entry for student ${student.id}:`,
              error
            );
          }
        }
        ...
        if (newEntryKey) {
          await set(
            ref(
              db,
              `diary/${schoolCode}/${currentGrade}/${selectedStudent}/${newEntryKey}`
            ),
            newEntryData
          );
        }
      ...
      <View style={styles.splitContainer}>
        ...
        <View style={styles.diaryContainer}>
          {selectedStudent ? (
              ...
              <ScrollView style={styles.entriesList}>
                {diaryEntries.map((entry) => (
                  <View key={entry.id} style={styles.entryCard}>
                    {selectedStudent === "all" && (
                      <Text style={styles.entryStudent}>
                        {entry.studentName}
                      </Text>
                    )}
            ...
            <Text style={styles.selectStudentText}>
              Select a student to view their diary
            </Text>
          )}
        </View>
      </View>
